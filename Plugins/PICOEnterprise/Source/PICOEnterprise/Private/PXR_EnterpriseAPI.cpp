...
UObject* UPICOXRSystemAPI::WorldContext = nullptr;
...
extern "C" JNIEXPORT void  JNICALL Java_com_epicgames_unreal_GameActivity_JavaToCSetArUcoMarkerCallback(JNIEnv * env, jclass clazz, jobjectArray MarkArray)
{
	...
	for (int32 Index = 0; Index < NumMark; ++Index)
	{
		...
		float TrackingOriginHeight = UPICOXRHMDFunctionLibrary::PXR_GetTrackingOriginHeight();

		Result.MarkerTranslation = FVector(-Position.Z, Position.X, -Position.Y - TrackingOriginHeight);
		if (IsValid(UPICOXRSystemAPI::WorldContext))
		{
			UWorld* World = GEngine->GetWorldFromContextObject(UPICOXRSystemAPI::WorldContext, EGetWorldErrorMode::ReturnNull);
			if (!IsValid(World))
			{
