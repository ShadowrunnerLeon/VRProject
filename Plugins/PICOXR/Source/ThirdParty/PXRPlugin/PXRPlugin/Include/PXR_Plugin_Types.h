...
// imu data
typedef struct PxrBodyTrackingImu_
{
...
typedef struct PxrBodyTrackingPose_
{
	int64_t TimeStamp;                // time stamp of imu
...
typedef struct PxrBodyTrackingTransform_
{
...
typedef struct PxrBodyTrackingResult_
{
	PxrBodyTrackingTransform trackingdata[Pxr_ROLE_NUM];;
} PxrBodyTrackingResult;

typedef struct PxrFitnessBandConnectState_
{
	uint8_t num;
	uint8_t trackerID[12];
} PxrFitnessBandConnectState;

typedef struct BodyTrackingBoneLength_
{
	float headLen;
	float neckLen;
	float torsoLen;
...
typedef enum BodyTrackingAlgParamType_
{
	HUMAN_HEIGHT = 0,
	SWIFT_MODE = 1,
	BONE_PARAM = 2
} BodyTrackingAlgParamType;

typedef struct BodyTrackingAlgParam_
{
	int BodyJointSet;
	BodyTrackingBoneLength BoneLength;
} BodyTrackingAlgParam;
...
typedef struct PxrFaceTrackingData_ {
	int                         apiVersion;
	float* blendShapeWeight; // array size = 72
	int64_t                     timestamp;
	float                       laughingProb;
	bool                        eyeValid;
	bool                        faceValid;
